...
__host__ __device__    void trace_particle(particle_t& p, double* srange_arr, double* trange_arr, double* zrange_arr, double* quadpts_arr,
    ...
    while(p.t < tmax){
        ...
        // } 
        // ensure no overshoot
        if (next_save > p.t && p.t + p.dt > next_save) {
            p.dt = next_save - p.t;
            if(p.dt <= 0) exit(EXIT_FAILURE);
        }
...
__host__ __device__    void poincare_trace(particle_t& p, double* srange_arr, double* trange_arr, double* zrange_arr, double* quadpts_arr,
                         double tmax, double m, double q, double psi0, double* saw_srange_arr, int* saw_m_arr, int* saw_n_arr, double* saw_phihats_arr, double saw_omega, int saw_nharmonics, int idx, double* traj_buffer, double dt_save, double tol, int MAX_PUNCTURES, double* zetas_arr, double* omegas_arr, int num_planes){
    ...
    while(p.t < tmax && punctures < MAX_PUNCTURES){
        ...
        for(int curr_plane = 0; curr_plane < num_planes; curr_plane ++) {
            ...
            double zeta = zetas_arr[curr_plane];
            double omega = omegas_arr[curr_plane];
            double phase_last = last_z - omega * last_tnow;
            double phase_current = current_z - omega * current_tnow;
            
            double last_quotient = floor((phase_last-zeta)/(2*M_PI));
            ...
            if (last_quotient != curr_quotient) { // checks if zeta1 - omega * t1 < zeta + 2kpi < zeta_2 - omega t_2
                ...
                double adjusted_hit = zeta + 2*k*M_PI;

                // parametrize using LINE = last + x(current - last)
...
__global__ void poincare_kernel(particle_t* particles, double* srange_arr, double* trange_arr, double* zrange_arr, double* quadpts_arr,
                        double tmax, double m, double q, double psi0, int nparticles, double* saw_srange_arr, int* saw_m_arr, int* saw_n_arr, double* saw_phihats_arr, double saw_omega, int saw_nharmonics, double* poincare_buffer, double dt_save, double tol, int MAX_PUNCTURES, double* zetas_arr, double* omegas_arr, int num_planes){
    // added traj buffer
    int idx = threadIdx.x + blockIdx.x*blockDim.x;
    if(idx < nparticles){
        // printf("tracing particle %d\n", idx);
        poincare_trace(particles[idx], srange_arr, trange_arr, zrange_arr, quadpts_arr, tmax, m, q, psi0, saw_srange_arr, saw_m_arr, saw_n_arr, saw_phihats_arr, saw_omega, saw_nharmonics, idx, poincare_buffer, dt_save, tol, MAX_PUNCTURES, zetas_arr, omegas_arr, num_planes);
    }
}
...
extern "C" vector<double> poincare_plotting(py::array_t<double> quad_pts, py::array_t<double> srange,
    ...
    py::buffer_info zetas_buf = zetas.request();
    double* zetas_arr = static_cast<double*>(zetas_buf.ptr);
    ...
    */
    ...
    double* zetas_d;
    cudaMalloc((void**)&zetas_d, zetas.size()*sizeof(double));
    cudaMemcpy(zetas_d, zetas_arr, zetas.size()*sizeof(double), cudaMemcpyHostToDevice);
    ...
    poincare_kernel<<<nblks, nthreads>>>(particles_d, srange_d, trange_d, zrange_d, quadpts_d, tmax, m, q, psi0, nparticles, saw_srange_d, saw_m_d, saw_n_d, saw_phihats_d, saw_omega, saw_nharmonics, traj_d, dt_save, tol, MAX_PUNCTURES, zetas_d, omegas_d, num_planes);
